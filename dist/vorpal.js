...
const vorpal_commons_1 = __importDefault(require("./vorpal-commons"));
class Vorpal extends events_1.EventEmitter {
    constructor() {
        super();
        // Program version
        ...
        this.cmdHistory = new history_1.default(); // this.CmdHistoryExtension();
        ...
        this._delimiter = 'local@' + String(os_1.default.hostname()).split('.')[0] + '~$ ';
        ui_1.default.setDelimiter(this._delimiter);
     ...
     */
    _init() {
        ui_1.default.on('vorpal_ui_keypress', data => {
            this.emit('keypress', data);
            this._onKeypress(data.key, data.value);
        });
        this.use(vorpal_commons_1.default);
    }
    /**
    ...
    parse(argv, options) {
        ...
        const args = argv;
        let result = this;
        const catchExists = !(lodash_1.default.find(this.commands, { _catch: true }) === undefined);
        args.shift();
        args.shift();
        if (args.length > 0 || catchExists) {
            if (options.use === 'minimist') {
                result = minimist_1.default(args);
            }
            else {
    ...
    use(commands, options) {
        ...
        if (lodash_1.default.isFunction(commands)) {
            commands.call(this, this, options);
        }
        else if (lodash_1.default.isString(commands)) {
            /* eslint-disable-next-line @typescript-eslint/no-var-requires */
            return this.use(require(commands), options);
        }
        else {
            commands = lodash_1.default.isArray(commands) ? commands : [commands];
            for (const cmd of commands) {
                if (cmd.command) {
                    ...
                    if (cmd.options) {
                        cmd.options = lodash_1.default.isArray(cmd.options) ? cmd.options : [cmd.options];
    ...
    command(name, desc, opts) {
        ...
        const cmd = new command_1.default(cmdName, this);
        if (desc) {
     ...
     */
    mode(name, desc, opts) {
        return this.command(name, desc, lodash_1.default.extend(opts || {}, { mode: true }));
    }
    /**
     ...
     */
        ...
        return this.command(name, desc, lodash_1.default.extend(opts || {}, { catch: true }));
    }
    /**
    ...
    _hook(fn) {
        ...
        this._unhook = intercept_1.default(fn);
        this._hooked = true;
        return this;
    }
    /**
    ...
    localStorage(id) {
        ...
        const ls = new local_storage_1.default(id);
        lodash_1.default.forEach(['getItem', 'setItem', 'removeItem'], method => {
            ...
            this.session.getKeypressResult(key, value, function (err, result) {
                if (!err && result !== undefined) {
                    if (lodash_1.default.isArray(result)) {
                        const formatted = util_1.default.prettifyArray(result);
                        self.ui.imprint();
    ...
    prompt(options = {}, userCallback) {
        return new Promise(resolve => {
            ...
            if (ssn.isLocal()) {
                ui_1.default.setDelimiter(options.message || ssn.delimiter());
                prompt = ui_1.default.prompt(options, result => {
                    ui_1.default.setDelimiter(ssn.delimiter());
                    cb(result);
                });
            }
            else {
    ...
    _prompt(data = {}) {
        ...
        if (ui_1.default.midPrompt()) {
            return self;
        }
        const prompt = ui_1.default.prompt({
            type: 'input',
            name: 'command',
     ...
     */
    exec(cmd, args, cb) {
        cb = lodash_1.default.isFunction(args) ? args : cb;
        args = args || {};
    ...
    _exec(item) {
        ...
        if (match) {
            ...
            if (lodash_1.default.isString(item.args) || !lodash_1.default.isObject(item.args)) {
            ...
            for (let j = 0; j < item.pipes.length; ++j) {
                ...
                if (lodash_1.default.isString(commandParts.args) || !lodash_1.default.isObject(commandParts.args)) {
            ...
            if (item.args.options.help && lodash_1.default.isFunction(match._help)) {
            ...
            if (item.sync === true) {
                ...
                let error;
                try {
                    response = item.fn.call(new command_instance_1.CommandInstance({
            ...
            item.pipes = item.pipes.map(function (pipe) {
                return new command_instance_1.CommandInstance({
                    commandWrapper: item,
     ...
     */
    find(name) {
        return lodash_1.default.find(this.commands, { _name: name });
    }
    /**
    ...
    _commandHelp(command) {
        ...
        if (this._help !== undefined && lodash_1.default.isFunction(this._help)) {
        ...
        const commands = matches
            ...
            .map(cmd => {
            const args = cmd._args.map(arg => util_1.default.humanReadableArgName(arg)).join(' ');
            return [
                cmd._name +
        ...
        const counts = {};
        let groups = lodash_1.default.uniq(matches
            .filter(function (cmd) {
            return (String(cmd._name)
                .trim()
                .split(' ').length > commandMatchLength);
            ...
            const prefix = `    ${util_1.default.pad(cmd + ' *', width)}  ${counts[cmd]} sub-command${counts[cmd] === 1 ? '' : 's'}.`;
            return prefix;
        });
        ...
        const commandsString = commands.length < 1
            ...
            : '\n  Commands:\n\n' +
                commands
                    .map(function (cmd) {
                    const prefix = '    ' + util_1.default.pad(cmd[0], width) + '  ';
                    const suffixArr = wrap_ansi_1.default(cmd[1], descriptionWidth - 8).split('\n');
                    for (let i = 0; i < suffixArr.length; ++i) {
                        if (i !== 0) {
                            suffixArr[i] = util_1.default.pad('', width + 6) + suffixArr[i];
                        }
                    }
        ...
        return String(this._helpHeader(!!invalidString) + invalidString + commandsString + '\n' + groupsString)
            .replace(/\n\n\n/g, '\n\n')
            .replace(/\n\n$/, '\n');
    }
    _helpHeader(hideTitle) {
        const header = [];
        if (this._banner) {
            header.push(util_1.default.padRow(this._banner), '');
        }
        ...
        if (this._title && !hideTitle) {
            ...
            header.push(util_1.default.padRow(title));
            if (this._description) {
                const descWidth = process.stdout.columns * 0.75; // Only 75% of the screen
                header.push(util_1.default.padRow(wrap_ansi_1.default(this._description, descWidth)));
            }
        }
    ...
    getSessionById(id) {
        ...
        let ssn = lodash_1.default.find(this.server.sessions, { id });
        ...
        if (!ssn) {
            const sessions = {
                ...
                server: lodash_1.default.map(this.server.sessions, 'id')
            };
